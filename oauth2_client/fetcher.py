"""
Components used to fetch (obtain) an OAuth2 token from the auth provider.
Supported grant types:
- Client Credentials (e.g. internal service to service communication)
...
from labsterutils.date_time import datetime_to_float, float_to_datetime
...
def fetch_token(app):
    ...
    Args:
        app (oauth2_client.models.Application): app instance to obtain the token for
...
class Fetcher:
    ...
    def fetch_raw_token(self):
        """
        Fetch a token from auth provider. Exact object type and available
    ...
    def access_token_from_raw_token(self, raw_token):
        ...
        try:
            ...
            log.error(
                ...
                'specification - this can require logic changes on our side. Recieved data: %s',
                raw_token
            )
            raise
...
class JWTFetcher(Fetcher):
    ...
    def jwt_claim(self, expiration_s=150):
        """
        Build the JWT claim used to obtain token from auth provider.
    ...
    def _audience(self):
        """
        Dynamically derive claim audience (`aud:`) from the URI.
...
def expires_or_none(raw_token):
    """
    Determine token's expiry date if available. The RFC isn't strict about this, so aren't we.
    JWT Bearer grant type doesn't return expiration info at all.
    ...
    This is not critical, worst case we will just refresh the token too
    often or get an unauthorized error. Hence the best-effort approach.
    ...
    if 'expires_in' in raw_token:
        expires_in_s = raw_token['expires_in']
        expires = timezone.now() + timedelta(seconds=expires_in_s)
