...
class OAuth2Client(OAuth2Session):
    ...
    def __init__(self, token, expired_callback=None):
        ...
        :param expired_callback: (optional) function accepting one parameter, `Application`. It is
            called when TokenExpiredError is detected or 403 status code received. It should return
            a valid (not expired) oauth2_client.model.AccessToken object for communication with
            Resource Owner specified by `Application`.
            ...
            doesn't in case of the in-house auth provider). If the token expiry is detected more
    ...
    def _install_callback(self, _expired_callback):
        """
        ...
        :param _expired_callback: a function taking Application model object as argument. It fetches
            and stores a new AccessToken
        ...
        def wrapped_callback(app):
            if self.last_token_fetch_time \
                ...
                raise TokenExpiredError(
                    ...
                    "either: 1) your provider issues super short-living tokens; 2) problem with the"
                    " client" % self.REAUTH_LIMIT_SECONDS
                )
            new_token = _expired_callback(app)
    ...
    def request(self, method, url, *args, **kwargs):  # pylint: disable=arguments-differ
        ...
        try:
            # super().request can raise `TokenExpiredError`
...
def fetch_and_store_token(app):
    ...
    Arguments:
        app (oauth2_client.models.Application): oauth application
